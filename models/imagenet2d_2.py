...
class Mixup(nn.Module):
  ...
  def forward(self, X, Y):
      ...
      if len(X.shape) == 2:
          X = lambda_val.view(-1, 1) * X + (1 - lambda_val.view(-1, 1)) * X[perm]
      elif len(X.shape) == 3:
          X = lambda_val.view(-1, 1, 1) * X + (1 - lambda_val.view(-1, 1, 1)) * X[perm]
      else:
          X = lambda_val.view(-1, 1, 1, 1) * X + (1 - lambda_val.view(-1, 1, 1, 1)) * X[perm]

      if self.mixadd:
          Y = (Y + Y[perm]).clip(0, 1)
      else:
          Y = lambda_val.view(-1, 1) * Y + (1 - lambda_val.view(-1, 1)) * Y[perm]
...
class SpectrogramPreprocess(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(channels):
            ...
            specs.append(spec)
        spectrogram = torch.stack(specs, dim=1)  # Stack along the channels dimension
        return spectrogram
...
class Net(nn.Module):
    def __init__(self, cfg, device):
        ...
        # Pool
        if cfg.pool == "gem":
            self.global_pool = GeM(p_trainable=cfg.gem_p_trainable).to(self.device)
        ...
        # Head
        self.head = nn.Linear(self.backbone.num_features, self.num_classes).to(self.device)
        self.loss_fn = torch.nn.KLDivLoss(reduction='batchmean').to(self.device)
    ...
    def forward(self, batch):
        ...
        x = x.permute(0, 2, 1).reshape(bs * c, l)  # Move channels to batch dimension and prepare for MelSpectrogram
